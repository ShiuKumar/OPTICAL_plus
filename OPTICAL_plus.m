function pred_label = OPTICAL_plus(train_data,class_train,test_data,Fs,WindowSize,r)
...
Target_Test = Target_Test(sum(sum(isnan(Test_data)))==0);

Train_data = Train_data(:,:,sum(sum(isnan(Train_data)))==0);
Test_data = Test_data(:,:,sum(sum(isnan(Test_data)))==0);
